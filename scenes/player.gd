...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("q") and blaster_equiped and blaster_cooldown and !shooting:
		...
		# Elegir dirección de disparo según el mouse
		if abs(mouse_loc_from_player.x) > abs(mouse_loc_from_player.y):

			if mouse_loc_from_player.x > 0:
				last_direction = "right"
				sync_animation.rpc("shoot_right")
			else:
				last_direction = "left"
				sync_animation.rpc("shoot_left")

		else:

			if mouse_loc_from_player.y > 0:
				last_direction = "down"
				sync_animation.rpc("shoot_down")
			else:
				last_direction = "up"
				sync_animation.rpc("shoot_top")
		...
		# Mostrar sprite de disparo durante un instante
		await get_tree().create_timer(0.15).timeout

		shooting = false
		
		if velocity == Vector2.ZERO:
			sync_animation.rpc("idle_" + last_direction)
...
func get_input():
	...
	sync_animation.rpc("run_" + last_direction)
