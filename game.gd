...
# const WS_SERVER_URL := "ws://localhost:8080"   # solo pruebas locales
...
func _on_peer_connected(pid):
	if multiplayer.is_server():
		...
		# Avisar a todos que hay uno nuevo:
		anunciar_jugador.rpc(pid)
		# Avisar al nuevo quiénes ya estaban:
		for p in players:
			var pid_existente := int(p.name)
			if pid_existente != pid:
				anunciar_jugador.rpc_id(pid, pid_existente)
		sincronizar_diccionario.rpc_id(pid, PlayerData.players)

@rpc("reliable")
func anunciar_jugador(pid:int):
	if multiplayer.is_server():
		return
	...
	if get_node_or_null(str(pid)):
		return
	print("Spawn de respaldo para el peer ", pid)
...
func _remove_player(pid):
	...
	if player:
		...
		player.queue_free()
	pedir_eliminar_jugador.rpc(pid)

@rpc("call_local", "reliable")
func pedir_eliminar_jugador(pid:int):
	var player = get_node_or_null(str(pid))
	if player:
		players.erase(player)
		player.queue_free()
