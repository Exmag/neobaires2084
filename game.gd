...
func _servidor_heartbeat():
	...
	for pid in _last_seen.keys():
		if now - _last_seen.get(pid, now) > 20.0:
...
# ============ CREACIÓN / ELIMINACIÓN DE JUGADORES (única vía) ============

func _crear_jugador_local(pid:int):
	if get_node_or_null(str(pid)):
		return
	var player = PLAYER.instantiate()
	player.name = str(pid)
	var piso = $Level/Piso
	var spawn_point = piso.get_child(players.size() % piso.get_child_count())
...
func crear_jugador(pid:int):
	_crear_jugador_local(pid)
...
@rpc("reliable")
func sync_lista_jugadores(ids:Array):
	for pid in ids:
		_crear_jugador_local(int(pid))

func _on_peer_connected(pid):
	if multiplayer.is_server():
		_last_seen[pid] = Time.get_unix_time_from_system()
		# Al recién entrado le mandamos la lista completa (él incluido):
		var ids := []
		for p in players:
			ids.append(int(p.name))
		ids.append(pid)
		sync_lista_jugadores.rpc_id(pid, ids)
		# A todos les avisamos del nuevo:
		crear_jugador.rpc(pid)
...
func _on_session_created():
	...
	%Scoreboard.show()
	_crear_jugador_local(multiplayer.get_unique_id())
	registrar_nombre.rpc(
		multiplayer.get_unique_id(),
		get_player_name()
	)
