...
var _heartbeat_client_acum := 0.0
...
var _reintento_conexion_timer: Timer
var _animacion_puntos_timer: Timer
var _puntos_actuales := 0
...
func _process(delta):
	...
	if multiplayer.is_server():
		...
		_heartbeat_client_acum += delta
		if _heartbeat_client_acum >= 5.0:
			_heartbeat_client_acum = 0.0
...
func _crear_jugador_local(pid:int, idx:int):
	...
	var spawn_points = [
		$Level/Piso/SpawnPoint1,
		$Level/Piso/SpawnPoint2,
		$Level/Piso/SpawnPoint3,
		$Level/Piso/SpawnPoint4
	]

	var spawn_point = spawn_points[idx % spawn_points.size()]
	...
	player.set_spawn_position.rpc(spawn_pos)
...
func registrar_nombre(peer_id: int, nombre: String):
	...
	sincronizar_diccionario.rpc(PlayerData.players)
...
func _inmovilizar_jugadores():
	for id in PlayerData.players.keys():
		...
		if jugador:
			jugador.set_game_over.rpc(true)
...
func _on_spawner_spawned(node: Node):
	print("[SPAWNER] SPAWNED: ", node.name,
		" | peer=", multiplayer.get_unique_id())
	if node is Player and node.get_multiplayer_authority() == multiplayer.get_unique_id():
		node.dash_cooldown_started.connect(_on_dash_cooldown_started)
		node.shield_cooldown_started.connect(_on_shield_cooldown_started)
		node.invisibility_cooldown_started.connect(_on_invisibility_cooldown_started)

func _on_spawner_despawned(node: Node):
	print("[SPAWNER] DESPAWNED: ", node.name,
		" | peer=", multiplayer.get_unique_id())
		
func _empezar_intento_conexion() -> void:
	%Join.hide()
	%ConectandoLabel.text = "Iniciando servidor. Por favor espere."
	%ConectandoLabel.show()
	_puntos_actuales = 0

	conectar_websocket()

	if _reintento_conexion_timer == null:
		_reintento_conexion_timer = Timer.new()
		_reintento_conexion_timer.wait_time = 2.0
		_reintento_conexion_timer.timeout.connect(_on_reintento_conexion_timeout)
		add_child(_reintento_conexion_timer)
	_reintento_conexion_timer.start()

	if _animacion_puntos_timer == null:
		_animacion_puntos_timer = Timer.new()
		_animacion_puntos_timer.wait_time = 0.5
		_animacion_puntos_timer.timeout.connect(_on_animacion_puntos_timeout)
		add_child(_animacion_puntos_timer)
	_animacion_puntos_timer.start()

func _on_reintento_conexion_timeout() -> void:
	var conectado := multiplayer.multiplayer_peer != null \
		and multiplayer.multiplayer_peer.get_connection_status() == MultiplayerPeer.CONNECTION_CONNECTED
	if !conectado:
		conectar_websocket()

func _on_animacion_puntos_timeout() -> void:
	_puntos_actuales = (_puntos_actuales % 3) + 1
	%ConectandoLabel.text = "Iniciando servidor. Por favor espere" + ".".repeat(_puntos_actuales)

func _detener_intento_conexion() -> void:
	if _reintento_conexion_timer:
		_reintento_conexion_timer.stop()
	if _animacion_puntos_timer:
		_animacion_puntos_timer.stop()
	%ConectandoLabel.hide()
