...
func crear_jugador(pid:int, idx:int):
...
@rpc("reliable")
func sync_lista_jugadores(lista:Array):
	for entry in lista:
		_crear_jugador_local(int(entry[0]), int(entry[1]))
...
func _on_peer_connected(pid):
	if multiplayer.is_server():
		...
		var lista := []
		for p in players:
			var other := int(p.name)
			lista.append([other, _spawn_idx.get(other, 0)])
		lista.append([pid, idx])
		sync_lista_jugadores.rpc_id(pid, lista)
		crear_jugador.rpc(pid, idx)
